...
class DecisionTree(abc.ABC):
    ...
    def _find_best_criteria(self, feature_idx, node):
        ...
        raise NotImplementedError("Find_best_criteria method is not implemented for the base class. Use a subclass instead.")
    ...
    def _build_tree(self, node, depth):
        try:
            ...
            # Iterate over each feature index instead of using map()
            for feature_idx in range(node.data.shape[1]):
            ...
            if best_criteria:
                node.feature_index, node.threshold, left_mask, right_mask = best_criteria
                left_node = DecisionTreeNode(data=node.data[left_mask], labels=node.labels[left_mask])
                if left_node.num_samples > 0:
                    node.left = left_node
                    self._build_tree(node.left, depth + 1)
                right_node = DecisionTreeNode(data=node.data[right_mask], labels=node.labels[right_mask])
    ...
    def predict(self, X):
        raise NotImplementedError("Predict method is not implemented for the base class. Use a subclass instead.")

    def score(self, X, y):
        raise NotImplementedError("Score method is not implemented for the base class. Use a subclass instead.")

    def _predict_single(self, x):
        raise NotImplementedError("Predict_single method is not implemented for the base class. Use a subclass instead.")
...
class DecisionTreeClassifier(DecisionTree):
    ...
    def _find_best_criteria(self, feature_idx, node):
        ...
        left_counts: npt.NDArray[np.intp] = np.zeros(node.counts.shape[0], dtype=np.intp)
        right_counts: npt.NDArray[np.intp] = np.copy(node.counts)

        for idx, threshold in enumerate(thresholds):
            left_mask, right_mask = feature_values <= threshold, feature_values > threshold
            ...
            if left_label != right_label:
                left_y_size, right_y_size = node.data[left_mask].shape[0], node.data[right_mask].shape[0]
                ...
                weighted_left_impurity = (left_y_size / node.num_samples) * impurity_func(self.criterion, (left_y_size, left_input))
                weighted_right_impurity = (right_y_size / node.num_samples) * impurity_func(self.criterion, (right_y_size, right_input))

                impurity = weighted_left_impurity + weighted_right_impurity
                if impurity < best_impurity:
                    best_impurity = impurity
                    best_criteria = (feature_idx, threshold, left_mask.copy(), right_mask.copy())
    ...
    def _predict_single(self, x):
        node = self.tree
        while node and not node.is_leaf and not (node.left is None and node.right is None):
    ...
    def _find_best_criteria(self, feature_idx, node):
        ...
        for idx, threshold in enumerate(thresholds):
            ...
            if left_label != right_label and left_count > 0 and right_count > 0:
                ...
                right_impurity = (right_sum_sq - (right_sum ** 2) / right_count) / right_count

                impurity = (left_count / node.num_samples) * left_impurity + (right_count / node.num_samples) * right_impurity
                if impurity < best_impurity:
                    best_impurity = impurity
                    best_criteria = (feature_idx, threshold, left_mask.copy(), right_mask.copy())
